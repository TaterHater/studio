...
export default function HomePage() {
  ...
  // Assuming you have an auth context hook
  ...
  useEffect(() => {
    ...
    // Basic Firebase client SDK initialization (replace with your actual config)
    ...
    const fetchTrainDataA = async () => {
        const response = await fetch('https://gettraindata-lx7gkg5gfq-uc.a.run.app/');
    };
    ...
    const unsubscribe = onSnapshot(trainDataCollection, (snapshot) => {
      const trainData: any[] = [];
 ...
 trainData.push(doc.data());
    ...
    fetchTrainDataA(); // Fetch data on initial load
    const intervalId = setInterval(fetchTrainDataA, 30000); // Call function every 10 seconds
    return () => { unsubscribe(); clearInterval(intervalId); }; // Cleanup listener and interval on component unmount
    ...
    <main className="flex flex-col h-screen max-h-screen overflow-hidden bg-background">
      ...
      <div className="flex-grow flex flex-col overflow-hidden p-4 gap-4">
        <section aria-labelledby="map-view-heading" className="flex-1 min-h-0 flex flex-col">
          <h2 id="map-view-heading" className="sr-only">Live View</h2>
      ...
      <footer className="p-3 border-t text-center text-xs text-muted-foreground">
        <p>&copy; {new Date().getFullYear()}RTD Train Tracker. All rights reserved. Map data &copy; Google.</p>
